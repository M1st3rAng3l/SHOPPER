...
function Carousel(){
  ...
  // si la longitud de imageSourcesToDisplay es inferior a 4 imágenes que añaden las imágenes que faltan desde el comienzo de la matriz original
    ? [...ImageSources, ...images.slice(0, 4 - ImageSources.length) ]: ImageSources;

  return (
    <div>
    <div className="carouselBox">
      <img src={left} alt="iconLeft" className="btnIcon" onClick={prevIMage}/>
       {/* renderiza imagenes */}
       {imageSourcesToDisplay.map((image, index) =>
        <img key={index} src={image} alt="imagesCollection" className="imgCollection"/>
      )}
      <img src={right} alt='iconRight' className="btnIcon" onClick={nextImage}/>
      </div>
    </div>
  );
};
